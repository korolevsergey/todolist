...
jQuery.fn.validationEngine = function(settings) {
	if($.validationEngineLanguage){					// IS THERE A LANGUAGE LOCALISATION ?
		...
		allRules = {"required":{    			  // Add your regex rules here, you can take telephone as an example
							"regex":" ",
							"alertText":"The field can not be empty!",
						...
						"confirm":{
							"regex":"none",
							"alertText":"The passwords do not match!"},
						"same":{
							"regex":"none",
							"alertText":"Вы выбрали этот критерий более 1 раза!"},
						"telephone":{
							"regex":"/[0-9\-\+\ \]+$/",
							"alertText":"Некорректный номер телефона!"},	
						"email":{
							"regex":"/^[a-zA-Zа-яА-Я0-9_\.\-]+\@([a-zA-Zа-яА-Я0-9\-]+\.)+[a-zA-Zа-яА-Я0-9]{2,4}$/",
						...
						"onlyLetter":{
							"regex":"/^[а-я\А-Я\0-9\ \]+$/",
							"alertText":"Выберите значение!"},
					"onlyLatinLetter":{ "regex":"/^[a-zA-Z0-9]+$/",
							"alertText":"Only Latin letters and figures!"},
					"onlyCyrilicLetter":{ "regex":"/^[а-яА-Я\ \.\_\-]+$/",
							"alertText":"Только кириллица!"},
				  "onlyCyrilic":{ "regex":"/^[а-яА-Я\ \.\_\-]+$/",
							"alertText":"Только имена, без знаков припинания!"}
	...
	var validateCall = function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FILED
		...
		function _confirm(caller,rules,position){		 // VALIDATE FIELD MATCH
			...
			if($(caller).attr('value') != $("#"+confirmField).attr('value')){
				...
				function _same(caller,rules,position){		 // VALIDATE FIELD MATCH
			sameField = rules[position+1]
			
			if($(caller).attr('value') == $("#"+sameField).attr('value') && $(caller).attr('value')!=""){
				isError = true
				promptText += settings.allrules["same"].alertText+"<br />"
			}
		}
